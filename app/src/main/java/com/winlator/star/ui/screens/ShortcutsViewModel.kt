...
enum class ShortcutSortOrder { NAME_ASC, NAME_DESC, CONTAINER, RECENT }
...
class ShortcutsViewModel(app: Application) : AndroidViewModel(app) {
    ...
    val shortcuts: kotlinx.coroutines.flow.Flow<List<Shortcut>> =
        combine(_shortcuts, _sortOrder) { list, order ->
            when (order) {
                ...
                ShortcutSortOrder.RECENT     -> list.sortedByDescending { it.file.lastModified() }
            }
        }
    ...
    fun cloneShortcut(shortcut: Shortcut, newName: String, context: Context): Boolean {
        ...
        val container = shortcut.container ?: return false
        val desktopDir = container.getDesktopDir()
        if (!desktopDir.exists()) desktopDir.mkdirs()
        val safeName = trimmed.replace(Regex("""[\\/:*?"<>|]"""), "_")
        val newFile = File(desktopDir, "$safeName.desktop")
        if (newFile.exists()) {
            Log.w(TAG, "Clone target already exists: $newFile")
            return false
        }
        return try {
            val lines = FileUtils.readLines(shortcut.file).map { line ->
                if (line.startsWith("Name=")) "Name=$trimmed" else line
            }
            newFile.writeText(lines.joinToString("\n") + "\n")

            if (shortcut.iconFile != null && shortcut.iconFile.isFile) {
                val newIconFile = File(shortcut.iconFile.parentFile, "$safeName.png")
                shortcut.iconFile.copyTo(newIconFile, overwrite = true)
            }
            refresh()
            true
        } catch (e: Exception) {
            Log.e(TAG, "Failed to clone shortcut to $trimmed", e)
            false
        }
    }
