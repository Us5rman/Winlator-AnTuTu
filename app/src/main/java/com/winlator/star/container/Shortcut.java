...
public class Shortcut {
    ...
    private Bitmap bannerArt;
    private String customBannerPath; // Path to custom wide banner art

    private static final String COVER_ART_DIR = "app_data/cover_arts/"; // Removed leading "/" to keep it relative
    private static final String BANNER_ART_DIR = "app_data/banner_arts/";
    ...
    public Shortcut(Container container, File file) {
        ...
        this.customBannerPath = getExtra("customBannerPath");

        // Load cover art and banner art if available
        loadCoverArt();
        loadBannerArt();
    ...
    private void loadBannerArt() {
        if (customBannerPath != null && !customBannerPath.isEmpty()) {
            File customBannerFile = new File(customBannerPath);
            if (customBannerFile.isFile()) {
                this.bannerArt = BitmapFactory.decodeFile(customBannerFile.getPath());
                return;
            }
        }

        File defaultBannerFile = new File(BANNER_ART_DIR, this.name + ".png");
        if (defaultBannerFile.isFile()) {
            this.bannerArt = BitmapFactory.decodeFile(defaultBannerFile.getPath());
        }
    }
    ...
    // Getters and setters for bannerArt and customBannerPath — mirrors coverArt above.
    public Bitmap getBannerArt() {
        return bannerArt;
    }

    public void setBannerArt(Bitmap bannerArt) {
        this.bannerArt = bannerArt;
    }

    public String getCustomBannerPath() {
        return customBannerPath;
    }

    public void setCustomBannerPath(String customBannerPath) {
        this.customBannerPath = customBannerPath;
        putExtra("customBannerPath", customBannerPath);
        saveData();
        Log.d("Shortcut", "Set and saved custom banner path: " + customBannerPath);
    }
    ...
    /** Save the custom banner art to the banner art directory — mirrors saveCustomCoverArt. */
    public void saveCustomBannerArt(Bitmap bannerArt) {
        try {
            File bannerArtDir = new File(container.getRootDir(), BANNER_ART_DIR);
            if (!bannerArtDir.exists()) {
                boolean created = bannerArtDir.mkdirs();
                if (!created) {
                    Log.e("Shortcut", "Failed to create banner art directory: " + bannerArtDir.getAbsolutePath());
                }
            }

            File bannerFile = new File(bannerArtDir, this.name + ".png");
            if (FileUtils.saveBitmapToFile(bannerArt, bannerFile)) {
                this.bannerArt = bannerArt;
                setCustomBannerPath(bannerFile.getPath());
                Log.d("Shortcut", "Custom banner art saved at: " + bannerFile.getPath());
            } else {
                Log.e("Shortcut", "Failed to save custom banner art.");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    /** Removes the custom banner and falls back to the default — mirrors removeCustomCoverArt. */
    public void removeCustomBannerArt() {
        if (customBannerPath != null && !customBannerPath.isEmpty()) {
            File customBannerFile = new File(customBannerPath);
            Log.d("Shortcut", "Removing custom banner file at: " + customBannerPath);
            if (customBannerFile.exists() && customBannerFile.delete()) {
                Log.d("Shortcut", "Custom banner file deleted successfully.");
            } else {
                Log.e("Shortcut", "Failed to delete custom banner file or it doesn't exist.");
            }
        }

        this.customBannerPath = null;
        this.bannerArt = null;

        putExtra("customBannerPath", null);
        saveData();

        Log.d("Shortcut", "Shortcut state saved after removing custom banner. Current path: " + customBannerPath);
    }
